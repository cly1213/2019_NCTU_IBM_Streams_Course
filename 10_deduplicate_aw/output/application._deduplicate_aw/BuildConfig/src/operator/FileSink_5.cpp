...
void MY_OPERATOR_SCOPE::MY_OPERATOR::process(Tuple const & tuple$, uint32_t port)
{
	...
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_isbn()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_author()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_publisher()) << ',';
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("FileSink_5",&initer));
    return true;
}
