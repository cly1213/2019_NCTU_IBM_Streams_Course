// t4s4nameu2ids5titlef6salary


#ifndef BEJWRMSK2YUVMTS01YKWPNI3JLMLJTTMRTSXJLKOEAIZYAPX_H_
#define BEJWRMSK2YUVMTS01YKWPNI3JLMLJTTMRTSXJLKOEAIZYAPX_H_
...
#define SELF BeJwrMSk2yUvMTS01ykwpNi3JLMlJTTMrTsxJLKoEAIzYApX
...
public:
    ...
    typedef SPL::float32 salary_type;

    enum { num_attributes = 4 } ;
    
    SELF() : Tuple(), name_(), id_(), title_(), salary_() {}
    SELF(const Self & ot) : Tuple(), name_(ot.name_), id_(ot.id_), title_(ot.title_), salary_(ot.salary_) 
      { constructPayload(ot); }
    SELF(const name_type & _name, const id_type & _id, const title_type & _title, const salary_type & _salary) : Tuple(), name_(_name), id_(_id), title_(_title), salary_(_salary) { }
    ...
    {
        size_t size = sizeof(SPL::uint32)+sizeof(SPL::float32);
...
#undef SELF
#endif // BEJWRMSK2YUVMTS01YKWPNI3JLMLJTTMRTSXJLKOEAIZYAPX_H_ 
