...
{
    SPL::BeJwrMSk2yUvMTS01ykwpNi3JLMlJTTMrTsxJLKoEAIzYApX tuple$;
    double delay;
    ...
    try {
            ...
            failedRead = false;
                ...
                getPE().blockUntilShutdownRequest(delay);
                    ...
                    SPL::readCSV<SPL::float32,','>(fs, tuple$.get_salary());
                    
                CHECK_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_TUPLE("tuple<rstring name,uint32 id,rstring title,float32 salary>", _tupleNumber));
                CHECK_FAIL;
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("REC",&initer));
    return true;
}
