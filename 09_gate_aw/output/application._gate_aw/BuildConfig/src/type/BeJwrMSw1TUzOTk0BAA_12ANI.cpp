...
#include "BeJwrMSw1TUzOTk0BAA_12ANI.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<uint32 acked>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("acked");
        TypeOffset t(MY_OFFSETOF(acked_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "acked") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, acked_);
   else
     istr >> acked_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, acked_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "acked=", get_acked()) ;
    ostr << '}';
}
