...
#ifndef BEJWRMSW1TUZOTK0BAA_12ANI_H_
#define BEJWRMSW1TUZOTK0BAA_12ANI_H_
...
#define SELF BeJwrMSw1TUzOTk0BAA_12ANI
...
public:
    ...
    typedef SPL::uint32 acked_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), acked_() {}
    SELF(const Self & ot) : Tuple(), acked_(ot.acked_) 
      { constructPayload(ot); }
    SELF(const acked_type & _acked) : Tuple(), acked_(_acked) { }
    ...
    acked_type & get_acked() { return acked_; }
    const acked_type & get_acked() const { return acked_; }
    void set_acked(const acked_type & _acked) { acked_ = _acked; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<acked_type >()(acked_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMSW1TUZOTK0BAA_12ANI_H_ 
