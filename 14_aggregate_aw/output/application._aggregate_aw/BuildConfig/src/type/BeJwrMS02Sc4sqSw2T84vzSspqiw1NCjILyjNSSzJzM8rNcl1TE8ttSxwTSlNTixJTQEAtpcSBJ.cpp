...
#include "BeJwrMS02Sc4sqSw2T84vzSspqiw1NCjILyjNSSzJzM8rNcl1TE8ttSxwTSlNTixJTQEAtpcSBJ.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring city,rstring country,uint32 population,uint32 mAge,uint32 pEducated>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("population");
        TypeOffset t(MY_OFFSETOF(population_, tm), 
    ...
    {
        std::string s("mAge");
        TypeOffset t(MY_OFFSETOF(mAge_, tm), 
                     Meta::Type::typeOf<SPL::uint32 >(), 
                     &typeid(SPL::uint32));
        addMapping(*tm, t, s, 3);
    }
    ...
    {
        std::string s("pEducated");
        TypeOffset t(MY_OFFSETOF(pEducated_, tm), 
                     Meta::Type::typeOf<SPL::uint32 >(), 
                     &typeid(SPL::uint32));
        addMapping(*tm, t, s, 4);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "population") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, population_);
   else
     istr >> population_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "mAge") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, mAge_);
   else
     istr >> mAge_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "pEducated") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, pEducated_);
   else
     istr >> pEducated_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, population_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, mAge_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, pEducated_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "population=", get_population()) << ",";
    SPL::serializeWithPrecision(ostr << "mAge=", get_mAge()) << ",";
    SPL::serializeWithPrecision(ostr << "pEducated=", get_pEducated()) ;
    ostr << '}';
}
