...
{
    SPL::BeJwrMSo1T0ktKInPTCm2ADLi8xJzUwFW4wgQ tuple$;
    double delay;
    ...
    try {
            ...
            failedRead = false;
                ...
                getPE().blockUntilShutdownRequest(delay);
                    ...
                    SPL::readCSV<SPL::uint32,','>(fs, tuple$.get_dept_id());
                    
                        fs >> SPL::skipSpaceTabs;
                         
                            sep = fs.peek();
                            if (sep != ',') {
                                DO_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_CHAR (SPL::printableCharacter(','), SPL::printableCharacter(sep), _tupleNumber, "after reading the attribute dept_id"));
                            }
                            fs >> sep;
                        
                    SPL::readCSV<SPL::rstring,','>(fs, tuple$.get_dep_name());
                    
                CHECK_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_TUPLE("tuple<uint32 dept_id,rstring dep_name>", _tupleNumber));
                CHECK_FAIL;
		    ...
		    if (eolSep == EOF) {
                        ...
                        DO_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_CHAR ("\\n", SPL::printableCharacter(eolSep), _tupleNumber, "after reading the attribute dep_name"));
		    } 
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("Department",&initer));
    return true;
}
