// t4s4nameu4deptu7dept_ids8dep_name


#ifndef BEJWRMSK2YUVMTS01SUKTKCK1B5HXMSNFFKBGPEGCANQGAYW_H_
#define BEJWRMSK2YUVMTS01SUKTKCK1B5HXMSNFFKBGPEGCANQGAYW_H_
...
#define SELF BeJwrMSk2yUvMTS01SUktKCk1B5HxmSnFFkBGPEgCANQGAyw
...
public:
    ...
    typedef SPL::uint32 dept_type;
    typedef SPL::uint32 dept_id_type;
    typedef SPL::rstring dep_name_type;

    enum { num_attributes = 4 } ;
    
    SELF() : Tuple(), name_(), dept_(), dept_id_(), dep_name_() {}
    SELF(const Self & ot) : Tuple(), name_(ot.name_), dept_(ot.dept_), dept_id_(ot.dept_id_), dep_name_(ot.dep_name_) 
      { constructPayload(ot); }
    SELF(const name_type & _name, const dept_type & _dept, const dept_id_type & _dept_id, const dep_name_type & _dep_name) : Tuple(), name_(_name), dept_(_dept), dept_id_(_dept_id), dep_name_(_dep_name) { }
    ...
    dept_type & get_dept() { return dept_; }
    const dept_type & get_dept() const { return dept_; }
    void set_dept(const dept_type & _dept) { dept_ = _dept; }
    ...
    {
        buf << name_ << dept_ << dept_id_ << dep_name_;
    }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<dept_type >()(dept_);
    ...
    {
        size_t size = sizeof(SPL::uint32)+sizeof(SPL::uint32);
...
#undef SELF
#endif // BEJWRMSK2YUVMTS01SUKTKCK1B5HXMSNFFKBGPEGCANQGAYW_H_ 
