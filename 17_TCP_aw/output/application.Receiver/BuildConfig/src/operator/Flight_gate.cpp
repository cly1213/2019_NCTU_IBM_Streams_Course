...
void MY_OPERATOR_SCOPE::MY_OPERATOR::process(uint32_t) 
{
    ...
    SPL::BeJwrMS02T8upjM8rzS02T0ktcEksSS02yy_1KTM_1MKzZJSS0uKTZJB4oBADuUA9_1 tuple$;
...
while(!getPE().getShutdownRequested()) {
    ...
    try {
            ...
            failedRead = false;
                    ...
                    SPL::readCSV<SPL::rstring,','>(fs, tuple$.get_dest());
                        ...
                        fs >> SPL::skipSpaceTabs;
                            ...
                            sep = fs.peek();
                            if (sep != ',') {
                                DO_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_CHAR (SPL::printableCharacter(','), SPL::printableCharacter(sep), _tupleNumber, "after reading the attribute dest"));
                            }
                            fs >> sep;
                        
                    SPL::readCSV<SPL::rstring,','>(fs, tuple$.get_gate());
                    
                CHECK_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_TUPLE("tuple<rstring fly_num,rstring depDate,rstring origin,rstring dest,rstring gate>", _tupleNumber));
                CHECK_FAIL;
		    ...
		    if (eolSep == EOF) {
                        ...
                        DO_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_CHAR ("\\n", SPL::printableCharacter(eolSep), _tupleNumber, "after reading the attribute gate"));
		    } 
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("Flight_gate",&initer));
    return true;
}
