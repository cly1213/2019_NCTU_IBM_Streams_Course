...
void MY_OPERATOR_SCOPE::MY_OPERATOR::process(Tuple const & tuple$, uint32_t port)
{
    ...
    try {
        ...
        SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_a_num()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_c_bal()) << '\n';
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("Deposit",&initer));
    return true;
}
