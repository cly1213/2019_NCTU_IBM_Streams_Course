...
#ifndef BEJWRMXQZZ0STJ09KZAEAFQIDD7_H_
#define BEJWRMXQZZ0STJ09KZAEAFQIDD7_H_
...
#define SELF BeJwrMXQzz0stj09KzAEAFqIDD7
...
public:
    ...
    typedef SPL::float64 new_bal_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), new_bal_() {}
    SELF(const Self & ot) : Tuple(), new_bal_(ot.new_bal_) 
      { constructPayload(ot); }
    SELF(const new_bal_type & _new_bal) : Tuple(), new_bal_(_new_bal) { }
    ...
    new_bal_type & get_new_bal() { return new_bal_; }
    const new_bal_type & get_new_bal() const { return new_bal_; }
    void set_new_bal(const new_bal_type & _new_bal) { new_bal_ = _new_bal; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<new_bal_type >()(new_bal_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMXQZZ0STJ09KZAEAFQIDD7_H_ 
