...
#ifndef BEJWRMS41TYZPK811M02OT0RMCTM0SEKTIE_1MZS_1NKWEAMN0KCH_H_
#define BEJWRMS41TYZPK811M02OT0RMCTM0SEKTIE_1MZS_1NKWEAMN0KCH_H_
...
public:
    ...
    typedef SPL::float64 dep_amount_type;

    enum { num_attributes = 3 } ;
    
    SELF() : Tuple(), a_num_(), c_bal_(), dep_amount_() {}
    SELF(const Self & ot) : Tuple(), a_num_(ot.a_num_), c_bal_(ot.c_bal_), dep_amount_(ot.dep_amount_) 
      { constructPayload(ot); }
    SELF(const a_num_type & _a_num, const c_bal_type & _c_bal, const dep_amount_type & _dep_amount) : Tuple(), a_num_(_a_num), c_bal_(_c_bal), dep_amount_(_dep_amount) { }
    ...
    dep_amount_type & get_dep_amount() { return dep_amount_; }
    const dep_amount_type & get_dep_amount() const { return dep_amount_; }
    void set_dep_amount(const dep_amount_type & _dep_amount) { dep_amount_ = _dep_amount; }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<dep_amount_type >()(dep_amount_);
        return s;
    }
    ...
    { 
        ...
        std::swap(dep_amount_, ot.dep_amount_);
...
#undef SELF
#endif // BEJWRMS41TYZPK811M02OT0RMCTM0SEKTIE_1MZS_1NKWEAMN0KCH_H_ 
