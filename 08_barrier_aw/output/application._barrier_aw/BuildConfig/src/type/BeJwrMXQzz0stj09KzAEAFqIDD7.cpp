...
#include "BeJwrMXQzz0stj09KzAEAFqIDD7.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<float64 new_bal>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("new_bal");
        TypeOffset t(MY_OFFSETOF(new_bal_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "new_bal") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, new_bal_);
   else
     istr >> new_bal_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, new_bal_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "new_bal=", get_new_bal()) ;
    ostr << '}';
}
