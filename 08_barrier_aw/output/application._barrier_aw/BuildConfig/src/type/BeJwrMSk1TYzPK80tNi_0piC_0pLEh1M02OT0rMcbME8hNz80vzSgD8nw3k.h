// t4u5a_nums7tx_typeF5c_balF9tx_amount


#ifndef BEJWRMSK1TYZPK80TNI_0PIC_0PLEH1M02OT0RMCBME8HNZ80VZSGD8NW3K_H_
#define BEJWRMSK1TYZPK80TNI_0PIC_0PLEH1M02OT0RMCBME8HNZ80VZSGD8NW3K_H_
...
#define SELF BeJwrMSk1TYzPK80tNi_0piC_0pLEh1M02OT0rMcbME8hNz80vzSgD8nw3k
...
public:
    ...
    typedef SPL::rstring tx_type_type;
    typedef SPL::float64 c_bal_type;
    typedef SPL::float64 tx_amount_type;

    enum { num_attributes = 4 } ;
    
    SELF() : Tuple(), a_num_(), tx_type_(), c_bal_(), tx_amount_() {}
    SELF(const Self & ot) : Tuple(), a_num_(ot.a_num_), tx_type_(ot.tx_type_), c_bal_(ot.c_bal_), tx_amount_(ot.tx_amount_) 
      { constructPayload(ot); }
    SELF(const a_num_type & _a_num, const tx_type_type & _tx_type, const c_bal_type & _c_bal, const tx_amount_type & _tx_amount) : Tuple(), a_num_(_a_num), tx_type_(_tx_type), c_bal_(_c_bal), tx_amount_(_tx_amount) { }
    ...
    tx_type_type & get_tx_type() { return tx_type_; }
    const tx_type_type & get_tx_type() const { return tx_type_; }
    void set_tx_type(const tx_type_type & _tx_type) { tx_type_ = _tx_type; }
    ...
    tx_amount_type & get_tx_amount() { return tx_amount_; }
    const tx_amount_type & get_tx_amount() const { return tx_amount_; }
    void set_tx_amount(const tx_amount_type & _tx_amount) { tx_amount_ = _tx_amount; }
    ...
    {
        buf << a_num_ << tx_type_ << c_bal_ << tx_amount_;
    }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<tx_type_type >()(tx_type_);
        s = 37 * s + std::tr1::hash<c_bal_type >()(c_bal_);
        s = 37 * s + std::tr1::hash<tx_amount_type >()(tx_amount_);
        return s;
    }
    ...
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::uint32)+sizeof(SPL::float64)+sizeof(SPL::float64);
           size += tx_type_.getSerializedSize();
    ...
    { 
        a_num_ = ot.a_num_;
        tx_type_ = ot.tx_type_;
        c_bal_ = ot.c_bal_;
        tx_amount_ = ot.tx_amount_; 
        assignPayload(ot);
        return *this; 
    }
    ...
    { 
        ...
        std::swap(tx_type_, ot.tx_type_);
        std::swap(c_bal_, ot.c_bal_);
        std::swap(tx_amount_, ot.tx_amount_);
...
private:
    ...
    a_num_type a_num_;
    tx_type_type tx_type_;
    c_bal_type c_bal_;
    tx_amount_type tx_amount_;
...
#undef SELF
#endif // BEJWRMSK1TYZPK80TNI_0PIC_0PLEH1M02OT0RMCBME8HNZ80VZSGD8NW3K_H_ 
