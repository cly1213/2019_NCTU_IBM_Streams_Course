...
#include "BeJwrMS41TYzPK811M02OT0rMcTM0SElNik_1MzS_1NKwEAmg0KCT.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<uint32 a_num,float64 c_bal,float64 deb_amount>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("deb_amount");
        TypeOffset t(MY_OFFSETOF(deb_amount_, tm), 
                     Meta::Type::typeOf<SPL::float64 >(), 
                     &typeid(SPL::float64));
        addMapping(*tm, t, s, 2);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "deb_amount") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, deb_amount_);
   else
     istr >> deb_amount_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, deb_amount_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "deb_amount=", get_deb_amount()) ;
    ostr << '}';
}
