...
{
    SPL::BeJwrMSk2yUvMTS02LcksyUktNjRISS1ILCrJTc0rcTMrTsxJLKoEAPpZA3L tuple$;
...
while(!getPE().getShutdownRequested()) {
    ...
    try {
            ...
            failedRead = false;
                    ...
                    SPL::readCSV<SPL::rstring,','>(fs, tuple$.get_title());
                    
                        fs >> SPL::skipSpaceTabs;
                         
                            sep = fs.peek();
                            if (sep != ',') {
                                DO_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_CHAR (SPL::printableCharacter(','), SPL::printableCharacter(sep), _tupleNumber, "after reading the attribute title"));
                            }
                            fs >> sep;
                        
                    SPL::readCSV<SPL::rstring,','>(fs, tuple$.get_department());
                    
                        fs >> SPL::skipSpaceTabs;
                         
                            sep = fs.peek();
                            if (sep != ',') {
                                DO_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_CHAR (SPL::printableCharacter(','), SPL::printableCharacter(sep), _tupleNumber, "after reading the attribute department"));
                            }
                            fs >> sep;
                        
                    SPL::readCSV<SPL::float64,','>(fs, tuple$.get_salary());
                    
                CHECK_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_TUPLE("tuple<rstring name,rstring title,rstring department,float64 salary>", _tupleNumber));
                CHECK_FAIL;
		    ...
		    if (eolSep == EOF) {
                        ...
                        DO_ERROR (SPL_APPLICATION_RUNTIME_FAILED_READ_CHAR ("\\n", SPL::printableCharacter(eolSep), _tupleNumber, "after reading the attribute salary"));
		    } 
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("FileSource_1_out0",&initer));
    return true;
}
