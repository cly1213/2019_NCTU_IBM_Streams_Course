...
#ifndef BEJWRMSK2YUVMTS02LCKSYUKTNJRISS1ILCRJTC0RCTMRTSXJLKOEAPPZA3L_H_
#define BEJWRMSK2YUVMTS02LCKSYUKTNJRISS1ILCRJTC0RCTMRTSXJLKOEAPPZA3L_H_
...
public:
    ...
    typedef SPL::rstring department_type;
    typedef SPL::float64 salary_type;

    enum { num_attributes = 4 } ;
    
    SELF() : Tuple(), name_(), title_(), department_(), salary_() {}
    SELF(const Self & ot) : Tuple(), name_(ot.name_), title_(ot.title_), department_(ot.department_), salary_(ot.salary_) 
      { constructPayload(ot); }
    SELF(const name_type & _name, const title_type & _title, const department_type & _department, const salary_type & _salary) : Tuple(), name_(_name), title_(_title), department_(_department), salary_(_salary) { }
    ...
    department_type & get_department() { return department_; }
    const department_type & get_department() const { return department_; }
    void set_department(const department_type & _department) { department_ = _department; }
    salary_type & get_salary() { return salary_; }
    const salary_type & get_salary() const { return salary_; }
    void set_salary(const salary_type & _salary) { salary_ = _salary; }
    ...
    {
        buf << name_ << title_ << department_ << salary_;
    }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<department_type >()(department_);
        s = 37 * s + std::tr1::hash<salary_type >()(salary_);
        return s;
    }
   ...
   size += department_.getSerializedSize();
    ...
    { 
        ...
        std::swap(department_, ot.department_);
        std::swap(salary_, ot.salary_);
...
#undef SELF
#endif // BEJWRMSK2YUVMTS02LCKSYUKTNJRISS1ILCRJTC0RCTMRTSXJLKOEAPPZA3L_H_ 
