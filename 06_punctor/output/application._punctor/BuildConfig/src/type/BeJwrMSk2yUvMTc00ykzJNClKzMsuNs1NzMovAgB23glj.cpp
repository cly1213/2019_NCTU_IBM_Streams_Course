...
#include "BeJwrMSk2yUvMTc00ykzJNClKzMsuNs1NzMovAgB23glj.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring name,int32 id,int32 rank,rstring major>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("rank");
        TypeOffset t(MY_OFFSETOF(rank_, tm), 
                     Meta::Type::typeOf<SPL::int32 >(), 
                     &typeid(SPL::int32));
        addMapping(*tm, t, s, 2);
    }
    ...
    {
        std::string s("major");
        TypeOffset t(MY_OFFSETOF(major_, tm), 
                     Meta::Type::typeOf<SPL::rstring >(), 
                     &typeid(SPL::rstring));
        addMapping(*tm, t, s, 3);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "rank") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, rank_);
   else
     istr >> rank_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "major") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, major_);
   else
     istr >> major_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, rank_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, major_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "rank=", get_rank()) << ",";
    SPL::serializeWithPrecision(ostr << "major=", get_major()) ;
    ostr << '}';
}
