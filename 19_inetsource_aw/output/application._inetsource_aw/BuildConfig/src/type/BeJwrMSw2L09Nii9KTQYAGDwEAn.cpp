...
#include "BeJwrMSw2L09Nii9KTQYAGDwEAn.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring web_rec>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("web_rec");
        TypeOffset t(MY_OFFSETOF(web_rec_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "web_rec") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, web_rec_);
   else
     istr >> web_rec_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, web_rec_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "web_rec=", get_web_rec()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_web_rec());
