...
#ifndef BEJWRMSW2L09NII9KTQYAGDWEAN_H_
#define BEJWRMSW2L09NII9KTQYAGDWEAN_H_
...
#define SELF BeJwrMSw2L09Nii9KTQYAGDwEAn
...
public:
    ...
    typedef SPL::rstring web_rec_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), web_rec_() {}
    SELF(const Self & ot) : Tuple(), web_rec_(ot.web_rec_) 
      { constructPayload(ot); }
    SELF(const web_rec_type & _web_rec) : Tuple(), web_rec_(_web_rec) { }
    ...
    web_rec_type & get_web_rec() { return web_rec_; }
    const web_rec_type & get_web_rec() const { return web_rec_; }
    void set_web_rec(const web_rec_type & _web_rec) { web_rec_ = _web_rec; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<web_rec_type >()(web_rec_);
        return s;
    }
    ...
    {
        size_t size = 0;
           size += web_rec_.getSerializedSize();
...
#undef SELF
#endif // BEJWRMSW2L09NII9KTQYAGDWEAN_H_ 
