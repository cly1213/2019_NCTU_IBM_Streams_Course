...
#ifndef BEJWRMSK2K67MTCRPKBZISSXJLCNMTXUZLSJKTE4TNS7LZWEAY4AMBN_H_
#define BEJWRMSK2K67MTCRPKBZISSXJLCNMTXUZLSJKTE4TNS7LZWEAY4AMBN_H_
...
public:
    ...
    typedef SPL::uint32 vol_type;

    enum { num_attributes = 4 } ;
    
    SELF() : Tuple(), symbol_(), datetime_(), price_(), vol_() {}
    SELF(const Self & ot) : Tuple(), symbol_(ot.symbol_), datetime_(ot.datetime_), price_(ot.price_), vol_(ot.vol_) 
      { constructPayload(ot); }
    SELF(const symbol_type & _symbol, const datetime_type & _datetime, const price_type & _price, const vol_type & _vol) : Tuple(), symbol_(_symbol), datetime_(_datetime), price_(_price), vol_(_vol) { }
    ...
    vol_type & get_vol() { return vol_; }
    const vol_type & get_vol() const { return vol_; }
    void set_vol(const vol_type & _vol) { vol_ = _vol; }
    ...
    {
        buf << symbol_ << datetime_ << price_ << vol_;
    }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<vol_type >()(vol_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMSK2K67MTCRPKBZISSXJLCNMTXUZLSJKTE4TNS7LZWEAY4AMBN_H_ 
